...
const LEVEL_TONES: Record<MaintenanceAlertLevel, StatusTone> = {
  Upcoming: 'warning',
  Overdue: 'danger',
};
const STATUS_TONES: Record<MaintenanceAlertStatus, StatusTone> = {
  Open: 'danger',
...
export function MaintenanceAlertsTab() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: ({ signal }) =>
      apiClient.getPaginated<MaintenanceAlertRow>(
        '/api/maintenance-alerts',
        { status: statusFilter, limit: 50, sort: '-createdAt' },
        signal,
      ),
  });
  ...
  async function acknowledge(alert: MaintenanceAlertRow) {
    try {
      ...
      toast.error(
        error instanceof ApiError ? error.message : t('maintenance.acknowledgeFailedToast'),
      );
    }
  }
  ...
  const columns: ColumnDef<MaintenanceAlertRow, any>[] = [
    ...
    columnHelper.accessor('level', {
      ...
      cell: (info) => {
        ...
        return (
          <Badge
            label={t(
              `maintenance.level${level}` as
                'maintenance.levelUpcoming' | 'maintenance.levelOverdue',
            )}
    ...
    columnHelper.accessor('status', {
      ...
      cell: (info) => {
        ...
        return (
          <Badge
            label={t(
              `maintenance.status${status.replace(' ', '')}` as
                | 'maintenance.statusOpen'
                | 'maintenance.statusAcknowledged'
                | 'maintenance.statusResolved',
            )}
    ...
    columnHelper.accessor('dueAtMeter', {
      header: t('maintenance.columnDueAtMeter'),
