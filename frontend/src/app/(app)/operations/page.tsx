...
function OperationsPageContent() {
  ...
  const columns: ColumnDef<OperationLogRow, any>[] = [
    ...
    columnHelper.accessor((row) => `${row.startMeter} → ${row.endMeter}`, { id: 'meters', header: 'Start / End', enableSorting: false, cell: (info) => <span className="tabular-data">{info.getValue()}</span> }),
    columnHelper.accessor('operatingHours', { header: 'Hours', cell: (info) => <span className="tabular-data">{info.getValue()}</span> }),
    columnHelper.accessor('downtimeHours', { header: 'Downtime', cell: (info) => <span className="tabular-data">{info.getValue()}</span> }),
    ...
    createActionsColumn<OperationLogRow>((row) => (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" className="size-7" aria-label={`Actions for ${row.generator.code} on ${formatDate(row.date)}`}>
  ...
  return (
    <>
      <PageHeader
        title="Operations"
        description="Daily meter readings — the ground truth for billable hours and utilization."
        action={
          canWrite ? (
      ...
      <DataTable
        ...
        emptyTitle="No operation logs yet"
        emptyDescription={canWrite ? 'Log the first meter reading to start tracking operating hours.' : 'Entries will appear here once logged.'}
      />
